...
//Agregar uniones tuberias
var numero_uniones = 0;
function agregar_tuberias() {
  if (numero_uniones < 1) {
    cargarImagenes(50, 125);
    numero_uniones += 1;
  } else if (numero_uniones < 2) {
    cargarImagenes(50, 450);
    numero_uniones += 1;
  }
}
...
function cargarImagenes(posXc, posYc) {
  // Cargar imágenes aquí
  // Ejemplo:
  var imagen1 = new Image();
  imagen1.src = "../assets/conector.png";

  // Luego de cargar las imágenes, llamar a la función para agregar objetos con las imágenes
  imagen1.onload = function () {
    agregarObjeto(imagen1, posXc, posYc, imagen1.width, imagen1.height);
  };
}

function agregarObjeto(imagen, x, y, width, height) {
  objetos.push({
    imagen: imagen,
    x: x,
    y: y,
...
function actualizar_canvas() {
  ...
  for (var a = 0; a < objetos.length; a++) {
    ...
    // Dibujar puntos rojos en el borde derecho, izquierdo e inferior
    ctx.fillStyle = "red";

    // Punto en el borde derecho
    ctx.fillRect(
      objetos[a].x + objetos[a].width, // Posición X del borde derecho
      objetos[a].y, // Posición Y
      12, // Ancho del punto
      12 // Alto del punto
    );

    // Punto en el borde izquierdo
    ctx.fillRect(
      objetos[a].x, // Posición X del borde izquierdo
      objetos[a].y, // Posición Y
      12, // Ancho del punto
      12 // Alto del punto
    );

    // Punto en el borde inferior
    ctx.fillRect(
      objetos[a].x + objetos[a].width / 2.3, // Posición X centrada horizontalmente
      objetos[a].y + objetos[a].height, // Posición Y del borde inferior
      12, // Ancho del punto
      12 // Alto del punto
    );
  }
...
function puntos_testing() {
  ctx.fillRect(300, 220, 200, 200);
}
